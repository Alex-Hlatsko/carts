...
export function MaterialSelector({ selectedMaterials, availableMaterials, onSelectionChange }: MaterialSelectorProps) {
  ...
  const selectedMaterialsData = selectedMaterials.map(materialId => 
  ...
  const removeMaterial = (index: number) => {
    ...
    onSelectionChange(newMaterials);
  };
      ...
      {selectedMaterialsData.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {selectedMaterialsData.map((material, index) => (
            <Card key={`${material.id}-${index}`} className="relative overflow-hidden">
            ...
            <div className="space-y-4">
              ...
              {filteredMaterials.length === 0 && (
                <p className="text-center text-muted-foreground py-8">
                  {searchTerm ? 'Материалы не найдены' : 'Нет доступных материалов'}
                </p>
              )}
