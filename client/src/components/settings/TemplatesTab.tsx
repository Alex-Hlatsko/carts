...
export function TemplatesTab() {
  ...
  const fetchData = async () => {
    try {
      console.log('TemplatesTab: Fetching data...');
      setLoading(true);
      ...
      console.log('TemplatesTab: Data fetched', { templatesData, materialsData });
      setTemplates(templatesData);
      setMaterials(materialsData);
    } catch (error) {
      console.error('TemplatesTab: Error fetching data:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleEdit = (template: StandTemplate) => {
    console.log('TemplatesTab: Editing template:', template);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      console.log('TemplatesTab: Deleting template:', id);
      await deleteTemplate(id);
      fetchData();
    } catch (error) {
      console.error('TemplatesTab: Error deleting template:', error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('TemplatesTab: Saving template:', formData);
      ...
      console.error('TemplatesTab: Error saving template:', error);
            ...
            <div key={template.id} className="border rounded-lg p-3 sm:p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[1, 2, 3].map((shelfNumber) => {
                  const shelf = template.shelves?.find(s => s.number === shelfNumber);
                ...
                {formData.shelves.map((shelf) => (
                  <div key={shelf.number} className="border rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      <Select 
                        onValueChange={(value) => addMaterialToShelf(shelf.number, value)}
                        value=""
                      >
                        <SelectTrigger>
